...
class DQN(object):
    def __init__(self, n_states, n_actions, n_hidden, batch_size, lr, epsilon, gamma, target_replace_iter, memory_capacity, use_gpu):
    ...
    def learn(self):
        ...
        q_eval = self.eval_net(b_state).gather(0, b_action)
...
n_states = np.prod(env.observation_space.shape[:])  # 76800=1*160*160*3
...
# 建立 DQN
dqn = DQN(n_states, n_actions, n_hidden, batch_size, lr, epsilon,
          gamma, target_replace_iter, memory_capacity, use_gpu)

# load model
model_list = glob(os.path.join('result', 'model', '*.pt'))
start_iter = 0
if load_model and not len(model_list) == 0:
    model_list.sort()
    start_iter = int(model_list[-1].split('_')[-1].split('.')[0])
    dqn.load(os.path.join('result', 'model'), start_iter)
...
for i_episode in range(start_iter, n_episodes):
    ...
    while True:
        ...
        if done:
            print('Episode {} finished after {} timesteps. Total rewards = {}. Total time = {:.2f}s'.format(
    ...
    if i_episode % save_freq == 0:
        dqn.save(os.path.join('trained_models'), i_episode)
